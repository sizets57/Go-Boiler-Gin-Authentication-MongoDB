...
class DigitalIpom:
    def __init__(self):
        ...
        self.logger.info(
            "Reading config file for fetching Digital IPOM related data...",
            extra={"stage": "1"},
        )
        config.read(path)
    ...
    def astrum_login(self):
        payload = json.dumps(
            {"email": self.astrum_user, "password": self.astrum_password}
        )
    ...
    def get_ipom_details(self, token, ipom_id):
        ...
        url = self.base_url.rstrip("/") + f"/ipom/show/{ipom_id}?config=Nhcc%20Dev"
        response = requests.get(url, headers=headers)
    ...
    def OracleConnection(self, username, password, hostname, port, service=None):
        ...
        try:
            ...
            self.logger.error(
                f"Failed for Oracle DB: {str(e)}", exc_info=True, extra={"stage": "5"}
            )
            raise
    ...
    def SQLConnection(self, serverName, dbName, user, pwd):
        ...
        try:
            ...
            self.logger.error(
                f"Failed for SQL Server: {str(e)}", exc_info=True, extra={"stage": "5"}
            )
            raise
    ...
    def build_condition(self, df, cond):
        ...
        if column not in df.columns:
            self.logger.error(
                f"Column '{column}' not found in DataFrame", extra={"stage": "7"}
            )
    ...
    def combine_conditions(self, conditions, logic):
        ...
        for cond in conditions[1:]:
            if logic == "any":
                ...
                self.logger.error(
                    f"Invalid condition logic: {logic}", extra={"stage": "7"}
                )
    ...
    def decrypt(self, enc_text, key="AhzUuSqlk0v0WVXDmtPu0xSTFjsdRSpI"):
        ...
        try:
            ...
            self.logger.error(
                "Error occured decrypting password: {e}", extra={"stage": "4"}
            )
