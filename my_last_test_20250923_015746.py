...
ipom_id = "68d236d864de925e21f2d21d"
...
def apply_priority_rules(df, rules, ipom):
    ...
    for rule in rules:
        try:
            ...
            df.loc[idx, 'duedate'] = (now + timedelta(hours=int(rule['dueDate']))).strftime('%Y-%m-%d 18:59:00')
        
        except Exception:
            logger.warning(f"Failed applying rule: {rule['threshold']}", exc_info=True, extra={'stage': '9'})
    ...
    df.loc[fallback_idx, 'duedate'] = (now + timedelta(days=3)).strftime('%Y-%m-%d 18:59:00')
...
def get_data_from_db(token, details):
    # if not any(isinstance(handle, SQLHandler) for handle in logging.getLogger().handlers):
    ...
    engine = ipom_obj.get_db_engine(str(db_conn['type']), str(db_conn['userName']), db_conn_password, 
                                    str(db_conn['hostName']), str(db_conn['port']), str(db_conn['databaseName']))
...
def opening_tickets(token, details, rule_obj, df):
    ...
    condition_series = [
        ipom_obj.build_condition(df, cond)
        for cond in rule_obj.get('conditions', [])
    ]
    ...
    logger.info(f"Records after applying conditions: {len(filtered_df)}", extra={'stage': '8'})

    df_to_create = apply_priority_rules(filtered_df, details['priorityManagement'], ipom_obj)

    if len(df_to_create[df_to_create['priority'].isna()]) > 0:
        logger.critical(f"Records with no priority: {len(df_to_create[df_to_create['priority'].isna()])}", extra={'stage': '9'})
        print(df_to_create)
    ...
    if len(unique_user) != 1:
        logger.error(f"Expected one requester, got: {unique_user}", extra={'stage': '11'})
        raise ValueError(f"Expected one requester, got: {unique_user}")

    unique_org = df_to_create['organization'].unique()
    if len(unique_org) != 1:
        logger.error(f"Expected one organization, got: {unique_org}", extra={'stage': '11'})
...
def ipom_processor():
    ...
    for rule in rules:
        if rule['ruleName'] == 'Open':
            current_task = opening_tickets.override(task_id = f"{rule['ruleName']}_Tickets", 
                                    trigger_rule = TriggerRule.ALL_DONE, 
                                    execution_timeout = timedelta(minutes=20))\
                                    (token = token, details = details, 
                                    rule_obj = rule, df = data)
                            
        elif rule['ruleName'] == 'Close':
            current_task = close_tickets.override(task_id=f"{rule['ruleName']}_Tickets", 
                                    trigger_rule=TriggerRule.ALL_DONE, 
                                    execution_timeout=timedelta(minutes=20))()
        
        elif rule['ruleName'] == 'Reopen':
            current_task = reopen_tickets.override(task_id=f"{rule['ruleName']}_Tickets", 
                                    trigger_rule=TriggerRule.ALL_DONE, 
                                    execution_timeout=timedelta(minutes=20))()
            
        elif rule['ruleName'] == 'Updated at':
            current_task = update_tickets.override(task_id=f"Update_Tickets", 
                                    trigger_rule=TriggerRule.ALL_DONE, 
                                    execution_timeout=timedelta(minutes=20))()
